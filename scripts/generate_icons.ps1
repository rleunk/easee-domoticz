...
function Test-ChargerStrip([int]$x, [int]$y, [int]$Size, [double]$Ox = 0, [double]$Oy = 0, [double]$Scale = 1.0) {
    $s = ($Size / 48.0) * $Scale
    $oxs = $Ox * $s; $oys = $Oy * $s
...
function Get-SymbolPixelV2([string]$Kind, [int]$x, [int]$y, [int]$Size, [Drawing.Color]$Accent, [bool]$Dim) {
    ...
    $strip = if ($Dim) { [Drawing.Color]::FromArgb(200, 95, 98, 102) } else { [Drawing.Color]::FromArgb(255, 58, 62, 66) }
    ...
    $accent = Get-AccentColor $Accent $Dim

    switch ($Kind) {
        'charger' {
            if (Test-ChargerBody $x $y $Size) {
                if (Test-ChargerStrip $x $y $Size) { return $strip }
                return $body
            }
            if ((Test-EqualizerLed $x $y $Size) -and -not $Dim) { return $accent }
        }
        'equalizer' {
            if (Test-EqualizerPuck $x $y $Size) { return $puck }
            if (Test-EqualizerLed $x $y $Size) { return $accent }
        }
        'power' {
            if (Test-ChargerBody $x $y $Size) {
                if (Test-ChargerStrip $x $y $Size) {
                    if ($Dim) { return $strip }
                    $g = [Drawing.Color]::FromArgb(255,
                        [int](($accent.R + 46) / 2), [int](($accent.G + 160) / 2), [int](($accent.B + 67) / 2))
                    return $g
                }
                return $body
            }
        ...
        'overview' {
            foreach ($ox in @(-7, 7)) {
                if (Test-ChargerBody $x $y $Size -Ox $ox -Scale 0.72) {
                    if (Test-ChargerStrip $x $y $Size -Ox $ox -Scale 0.72) { return $strip }
                    return $body
                }
            }
        }
        'loadbal' {
            ...
            if (Test-EqualizerLed $x $y $Size -Scale 0.82) { return $accent }
        }
        'alert' {
            if (Test-ChargerBody $x $y $Size) {
                if (Test-ChargerStrip $x $y $Size) { return $red }
                return $body
            }
...
# Preview row (48px on-state icons)
$iconW = 48; $pad = 8
...
for ($i = 0; $i -lt $IconSets.Count; $i++) {
    $icon = New-EaseeIconV2 48 $IconSets[$i].Color $IconSets[$i].Kind $false
    $pg.DrawImage($icon, $pad + $i * ($iconW + $pad), $pad)
    $icon.Dispose()
}
$pg.Dispose()
