...
**Language / Taal:** [English](README.md) · **Nederlands** (this page)
...
> **Status (productie):** **`main`** = **1.1.7** — Easee charger observations API (Sep 2026), NL/EN UI, LB fase-detail; **11 tegels + LoadBal**. Zie [STABLE.nl.md](STABLE.nl.md), [VERSIONING.nl.md](VERSIONING.nl.md), [docs/nl/RELEASE_1.1.7.md](docs/nl/RELEASE_1.1.7.md).
>
...
>
> **Ontwikkeling:** branch **`v1`** voor toekomstige 1.2.x releases.
...
git checkout main   # productie 1.1.7; legacy v10: git checkout legacy/v10
sudo systemctl restart domoticz
```

In Domoticz: **Setup → Hardware → Python plugins** → **Easee Domoticz plugin v1 (1.1.7)** → Easee-gebruikersnaam + wachtwoord → **Create**.
...
> Git-authenticatie: [docs/nl/GIT_SETUP.md](docs/nl/GIT_SETUP.md) · English: [docs/GIT_SETUP.md](docs/GIT_SETUP.md) · Problemen: [docs/nl/TROUBLESHOOTING.md](docs/nl/TROUBLESHOOTING.md) · [docs/TROUBLESHOOTING.md](docs/TROUBLESHOOTING.md)
...
- Geen programmeerkennis nodig — scannable tegels; **Nederlands** (default) of **English** (Mode30)
...
| **Taal** | **Nederlands** (default) of **English** (Mode30) — tegelteksten en status |
| **Upgrade** | `git pull` + hardware herstarten; bij icon-wijzigingen zip opnieuw uploaden |

Verder: eigen namen per laadpaal (Mode2/3/4), state in `easee_state.json`, gestructureerde logging `[Easee v1.1.6][LEVEL]…`.

## v1 releases

| Versie | Status |
|--------|--------|
| **1.1.7** | **Stable productie** (`main`) — Easee charger observations API hotfix (Sep 2026) |
| **1.1.6** | Rollback — English UI; laadpaal `/state` kapot na Easee API cutoff |
| **1.1.5** | Rollback — LB fase-detail, soak bevestigd aug 2026 |
...
Details: [docs/nl/CONFIGURATION.md](docs/nl/CONFIGURATION.md).
...
| Poll interval (Mode1) | 30 sec | API-interval; zet op **60 sec** bij 429-waarschuwingen (zie [CONFIGURATION.md](docs/nl/CONFIGURATION.md)) |
...
Zie [docs/nl/CONFIGURATION.md](docs/nl/CONFIGURATION.md).
...
**Na elke upgrade:** herstart het Easee hardware-item in Domoticz. Zie [STABLE.nl.md](STABLE.nl.md) voor tags en rollback (legacy v10 op `legacy/v10`).
...
Stap-voor-stap: [INSTALL.nl.md](INSTALL.nl.md).

## Changelog & release

- Volledige geschiedenis: [CHANGELOG.md](CHANGELOG.md) — v1 start bij 0.1.0; legacy v10 hieronder in changelog
- **v1 productie:** [**v1.1.7**](https://github.com/rleunk/easee-domoticz/releases/tag/v1.1.7) op `main` · [**v1.1.6**](https://github.com/rleunk/easee-domoticz/releases/tag/v1.1.6) rollback
...
| Probleem | Zie |
|----------|-----|
| Plugin niet in lijst | [INSTALL.nl.md](INSTALL.nl.md) — pad + `python3-requests` |
| Geen iconen | [docs/nl/TROUBLESHOOTING.md](docs/nl/TROUBLESHOOTING.md) — custom iconen |
...
| 429 rate limit in log | Poll interval (Mode1) op **60 sec** — [TROUBLESHOOTING.md](docs/nl/TROUBLESHOOTING.md#http-429-rate-limit-easee-api)
| Verkeerd icoon op tegel | Upgrade naar v10.9.18+; zip opnieuw uploaden |

## Module structuur

Sinds v10.6.0 modulair; v1 (0.2.0+) voegt **`pricing/`** (prijsbronnen) en **`domoticz_energy_hints.py`** toe — **14** root-modules + `pricing/` (9 bestanden, **23** totaal). Overzicht: [docs/REFACTOR_MAPPING.md](docs/REFACTOR_MAPPING.md). Releases: [docs/RELEASE_1.1.6.md](docs/RELEASE_1.1.6.md), [docs/RELEASE_1.1.5.md](docs/RELEASE_1.1.5.md), [docs/RELEASE_1.0.0.md](docs/RELEASE_1.0.0.md).

## Problemen melden

[GitHub Issues](https://github.com/rleunk/easee-domoticz/issues) → **Bug melden** (Nederlands formulier). Vermeld pluginversie **v1.1.6** (main) of **v10.11.6** (legacy), Domoticz-versie en logregels `[Easee v…]` (geen wachtwoorden).

## Support & credits

- **Repo:** [github.com/rleunk/easee-domoticz](https://github.com/rleunk/easee-domoticz)
- **Installatie:** [INSTALL.nl.md](INSTALL.nl.md)
...
**Versie 1.1.7** (main) — Richard Leunk · Legacy v10: **v10.11.6** op branch `legacy/v10`
