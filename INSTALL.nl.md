...
> **Versielijnen:** branch **`main`** = productie **1.1.6**; branch **`v1`** = toekomstige ontwikkeling (1.2.x); legacy v10 op **`legacy/v10`** / tag **v10.11.6**. Zie [STABLE.nl.md](STABLE.nl.md) en [VERSIONING.nl.md](VERSIONING.nl.md).
...
Kies **Prijsbron (Mode9)** in hardware-instellingen. Zie [docs/nl/CONFIGURATION.md](docs/nl/CONFIGURATION.md) voor Tibber (Mode7), ENTSO-E (Mode24 + e-mail goedkeuring), EnergyZero (geen token) en Handmatig.
...
## Legacy v10 (branch `legacy/v10`)

| Item | Waarde |
|------|--------|
| Plugin | Easee Domoticz plugin v10.11.6 |
| Branch / tag | `legacy/v10` of `v10.11.6` / `v10.11.6-stable` |
| Kosten | Alleen via **Tibber** (Mode7) |
| Status | Bevroren — rollback / bestaande installaties |
...
- **HTTPS (standaard):** clone werkt direct; bij inlogprompt een Personal Access Token gebruiken — zie [docs/nl/GIT_SETUP.md](docs/nl/GIT_SETUP.md)
- **SSH (optioneel):** als je GitHub SSH-sleutels hebt geconfigureerd — zie [docs/nl/GIT_SETUP.md](docs/nl/GIT_SETUP.md) → *Optioneel: SSH*
...
Op branch **`v1`**: kies **Prijsbron (Mode9)** — **Geen** (alleen kWh), **Handmatig**, **Tibber**, **ENTSO-E** of **EnergyZero**. Zie [docs/nl/CONFIGURATION.md](docs/nl/CONFIGURATION.md).
...
- Zie [CHANGELOG.md](CHANGELOG.md) en [docs/nl/CONFIGURATION.md](docs/nl/CONFIGURATION.md)
...
### Specifiek: v10.11.x (legacy — branch `legacy/v10`)
...
- **v10.11.6** — Dag overzicht-migratie fix (`Device.Update(Name=…)` i.p.v. readonly direct assignment); idle timer **00:00**; legacy baseline.
- **v10.11.5** — Dag overzicht-migratie (legacy tegel hernoemen); idle timer **00:00** (readonly-fout op nieuwere Domoticz).
- **Legacy branch** — `legacy/v10` of tag `v10.11.6` / `v10.11.6-stable`; zie [STABLE.nl.md](STABLE.nl.md). Vorige stable tags blijven voor rollback.
...
- **Stable tag** — `v10.10.8-stable` voor rollback; zie [STABLE.nl.md](STABLE.nl.md).
...
3. Pas de afbeelding in [README.nl.md](README.nl.md) aan als je een andere bestandsnaam gebruikt.
...
| Symptoom | Oplossing |
|----------|-----------|
| Password not supported | Gebruik PAT of SSH — zie [docs/nl/GIT_SETUP.md](docs/nl/GIT_SETUP.md) |
| Permission denied (publickey) | SSH-sleutel toevoegen aan GitHub |
| Repository not found | Controleer repo-URL, SSH-sleutel of PAT (zie [docs/nl/GIT_SETUP.md](docs/nl/GIT_SETUP.md)) |
...
- [README.nl.md](README.nl.md) — overzicht en snelle start
- [docs/nl/GIT_SETUP.md](docs/nl/GIT_SETUP.md) — git-authenticatie (SSH / PAT)
